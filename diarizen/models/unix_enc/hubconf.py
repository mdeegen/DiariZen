...
def unix_enc_base(refresh=False, legacy=False, **kwargs):
    assert not legacy
    kwargs[
        "ckpt"
    ] = "/scratch/hpc-prf-nt2/deegen/deploy/forschung/DiariZen/unixenc/fairseq_models/cfg1/checkpoints/checkpoint_last.converted.pt"
    # ] = "/scratch4/lgarci27/hzili1/workspace/s3prl_csp/s3prl/upstream/unix_enc/fairseq_models/cfg1/checkpoints/checkpoint_last.converted.pt"
    return unix_enc_custom(refresh=refresh, legacy=legacy, **kwargs)

def unix_enc_base_sgl(refresh=False, legacy=False, **kwargs):
    assert not legacy
    kwargs[
        "ckpt"
    ] = "/scratch4/lgarci27/hzili1/workspace/s3prl_csp/s3prl/upstream/unix_enc/fairseq_models/cfg2/checkpoints/checkpoint_last.converted.pt"
    return unix_enc_custom(refresh=refresh, legacy=legacy, **kwargs)

def unix_enc_base_avgchs(refresh=False, legacy=False, **kwargs):
    assert not legacy
    kwargs[
        "ckpt"
    ] = "/scratch4/lgarci27/hzili1/workspace/s3prl_csp/s3prl/upstream/unix_enc/fairseq_models/cfg1/checkpoints/checkpoint_last.converted.pt"
    return unix_enc_custom(refresh=refresh, legacy=legacy, avg_channels=True, **kwargs)
