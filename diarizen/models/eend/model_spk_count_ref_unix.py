...
def load_unix_enc(load_unixenc):
...
class Model(BaseModel):
    def __init__(
        ...
        self.model = load_unix_enc(load_unixenc)
        ...
        # if film:
        #     self.film = FiLM(attention_in, film_dim)
    ...
    def forward(self, waveforms: torch.Tensor, gcpsd_features=None) -> torch.Tensor:
        """Pass forward
        ...
        # print("Waveform: ", waveforms.shape, flush=True)
        with torch.no_grad():
            unix_dict = self.model(waveforms, features_only=True)  # TODO outputs, Zeitliche auflösung gleich? Oder ist egal? daten processing vtl erwartet das aber  399?
            # {"x": x, "padding_mask": padding_mask, "features": features, "layer_results": layer_results}
            unix_feat = unix_dict["layer_results"]
            # print(type(unix_feat), flush=True)
        print("unix_feat: ", unix_feat.shape, flush=True)
        unix_feat = torch.stack(unix_feat, dim=-1)  # neue dim hinten
        unix_feat = torch.mean(unix_feat, dim=1)  # average across channels

        # try:
        #     print("unix_feat: ", unix_feat.shape, flush=True)  # TODO: nachguken wie layers ausgeben und dann weighted sum
        # except Exception as e:
        #     print("Error printing unix_feat shape: ", e, flush=True)

        unix_feat = self.weight_sum(unix_feat)
        unix_feat = torch.squeeze(unix_feat, -1)
        unix_feat = unix_feat.transpose(-2, -1)


        outputs = self.proj(unix_feat)  # TODO shape
        outputs = self.lnorm(outputs)

        # with torch.no_grad():
        #     gcpsd_emb = self.encoder(waveforms, gcpsd_features)

        # outputs = self.film(outputs, gcpsd_emb)

        outputs = self.conformer(outputs)  # , gcpsd_emb)
