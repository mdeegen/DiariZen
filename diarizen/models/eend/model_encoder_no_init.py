...
from diarizen.models.module.conformer import ConformerEncoder, gcc_encoder, FiLM, ConformerEncoder_film
...
class Model(BaseModel):
    def __init__(
        ...
        if self.conf:
            self.conformer2 = ConformerEncoder(
                attention_in=attention_in,
                ffn_hidden=ffn_hidden,
                num_head=num_head,
                num_layer=num_layer,
        ...
        self.gcc_encoder = gcc_encoder(
            ...
            linear_input_size=399,  # number of frames per channel
            linear_output_size=399,  # number of frames per channel
            num_head_aux=4,
            num_layer_aux=num_layer_aux,
            dropout=dropout,
            sin_cos=sin_cos,
        ...
        self.proj2 = nn.Linear(attention_in_aux, attention_in)
        self.lnorm2 = nn.LayerNorm(attention_in)
    ...
    def non_wavlm_parameters(self):
        return [
            ...
            *self.gcc_encoder.parameters(),
            # *self.conformer2.parameters(),
            *self.proj2.parameters(),
            *self.lnorm2.parameters(),
        ]
    ...
    def forward(self, waveforms: torch.Tensor, gcpsd_features) -> torch.Tensor:
        ...
        gcpsd_emb = self.gcc_encoder(gcpsd_features)

        gcpsd_emb = self.proj2(gcpsd_emb)
        gcpsd_emb = self.lnorm2(gcpsd_emb)
        if self.conf:
            gcpsd_emb = self.conformer2(gcpsd_emb)
