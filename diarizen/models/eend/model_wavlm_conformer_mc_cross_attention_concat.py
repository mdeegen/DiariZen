...
from diarizen.models.module.conformer import ConformerEncoder, gcc_encoder, cross_attn
...
class Model(BaseModel):
    def __init__(
        ...
        self.cross_attn = cross_attn(dim_main=attention_in, dim_aux=attention_in_aux)
        # self.merged_linear = nn.Linear(attention_in+attention_in, 2 * attention_in)


        self.classifier = nn.Linear(2*attention_in, self.dimension)
    ...
    def forward(self, waveforms: torch.Tensor, gcc_features=None) -> torch.Tensor:
        ...
        attn_out = self.cross_attn(outputs, gcc_embeddings)

        outputs = torch.cat((outputs, attn_out), dim=-1)
