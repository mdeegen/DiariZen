...
class Model(BaseModel):
    ...
    def forward(self, waveforms: torch.Tensor, gcc_features=None) -> torch.Tensor:
        ...
        """
        gcc_features = gcc_features.permute(0, 2, 3, 1)
        gcc_features = F.interpolate(gcc_features, size=(399, gcc_features.shape[-1]), mode='bilinear', align_corners=False)
        gcc_features = gcc_features.permute(0, 3, 1, 2)  # (batch, target_frames, channels, delays)
