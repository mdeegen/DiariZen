...
class Model(BaseModel):
    def __init__(
        ...
        if film:
            self.film = FiLM(attention_in, attention_in_aux)
        self.max_num_spk = max_num_spk

        self.spk_counting = gcc_encoder(
    ...
    def forward(self, waveforms: torch.Tensor, gcc_features) -> torch.Tensor:
        ...
        with torch.no_grad():
            emb = self.spk_counting(gcc_features)


        outputs = self.film(outputs, emb)

        outputs = self.conformer(outputs, emb)
