...
from diarizen.models.wavlm.wavlm_counting_pruned_att import Model as spk_counting
...
class Model(BaseModel):
    def __init__(
        ...
        self.encoder = spk_counting(
            chunk_size=chunk_size,
            max_speakers_per_chunk = max_speakers_per_frame,
            selected_channel = selected_channel,
            shared_wavlm_model = self.wavlm_model,
            )
    ...
    def forward(self, waveforms: torch.Tensor, gcpsd_features = None) -> torch.Tensor:
        ...
        with torch.no_grad():
            num_spk_logits = self.encoder(waveforms)
            num_spk = torch.argmax(num_spk_logits, dim=-1, keepdim=True)
            num_spk = num_spk.to(dtype=outputs.dtype)
