...
class Model(BaseModel):
    ...
    def __init__(
        ...
        if downstream_model == 'blstm':
            ...
            self.downstream = BLSTM(
                input_size=attention_in_downstream,
                hidden_size=hidden_size,
                num_layers=num_layer,)      
        elif downstream_model == 'blstm_stateful':
            self.downstream = BLSTM_stateful(
                input_size=attention_in_downstream,
                hidden_size=hidden_size,
                num_layers=num_layer,
            )
        elif downstream_model == 'blstm_torch':
            self.downstream = BLSTM_torch(
                input_size=attention_in_downstream,
                hidden_size=hidden_size,
                num_layers=num_layer,
            )
    ...
    def forward(self, waveforms: torch.Tensor) -> torch.Tensor:
        ...
        outputs = self.downstream(outputs, None)
