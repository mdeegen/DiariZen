...
from diarizen.models.encoder_only.wavlm_spk_counting import Model as spk_counting
...
class Model(BaseModel):
    def __init__(
        ...
        self.oracle_num_spk = oracle_num_spk
        if self.oracle_num_spk:
    ...
    def forward(self, waveforms, gcc_features) -> torch.Tensor:
        """Pass forward
        ...
        with torch.no_grad():
            num_spk = self.spk_counting(waveforms, gcc_features)
            # if not self.probs:
            num_spk = torch.argmax(num_spk, dim=-1, keepdim=True).float().detach()
