...
class Model(BaseModel):
    def __init__(
        ...
        self.lnorm_gcc = nn.LayerNorm(attention_in)

        # self.norm_wavLM = nn.LayerNorm(attention_in)
        # self.norm_gcc = nn.LayerNorm(attention_in_aux)

        self.merged_linear = nn.Linear(attention_in+attention_in, attention_in)
    ...
    def print_feature_debug_stats(self, wavlm_feat, gcc_embeddings, combined, step=None):
        def stats(x, name):
            # x: (batch, time, feat_dim)
            b, t, f = x.shape
            x_flat = x.reshape(-1, f)  # (batch*time, feat_dim)

            mean_per_feature = x_flat.mean(dim=0)
            std_per_feature = x_flat.std(dim=0)

            print(f"\n[Step {step}] {name} STATS:")
            print(f"  shape          : {x.shape}")
            print(f"  mean (avg)     : {mean_per_feature.mean().item():.7f}")
            print(f"  std  (avg)     : {std_per_feature.mean().item():.4f}")
            print(f"  mean (min/max) : {mean_per_feature.min().item():.2f} / {mean_per_feature.max().item():.2f}")
            print(f"  std  (min/max) : {std_per_feature.min().item():.2f} / {std_per_feature.max().item():.2f}")

        stats(wavlm_feat, "wavLM_feat")
        stats(gcc_embeddings, "GCC_emb")
        stats(combined, "Combined")

        # Optional: Cosine similarity (only if dims match)
        import torch.nn.functional as F
        if wavlm_feat.shape[-1] == gcc_embeddings.shape[-1]:
            sim = F.cosine_similarity(wavlm_feat.reshape(-1, wavlm_feat.shape[-1]),
                                      gcc_embeddings.reshape(-1, gcc_embeddings.shape[-1]), dim=-1)
            print(f"[Step {step}] Cosine similarity → mean: {sim.mean().item():.4f} | std: {sim.std().item():.4f}")
        return
    ...
    def forward(self, waveforms: torch.Tensor, gcc_features=None) -> torch.Tensor:
        ...
        gcc_embeddings = self.lnorm_gcc(gcc_embeddings)
        ...
        # with torch.no_grad():
        #     print(gcc_embeddings.mean(dim=0))
        #     self.print_feature_debug_stats(outputs, gcc_embeddings, combined,
