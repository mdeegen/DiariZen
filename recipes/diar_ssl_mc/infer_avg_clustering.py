...
def diarize_session(
    ...
    # TODO: Comment this line for MC signals in inference
    ...
    print(f'segmentations: {segmentations.data.shape}')
    cached_dir = os.path.join(out_dir, "cached_segmentations")
    Path(cached_dir).mkdir(exist_ok=True, parents=True)
    segmentations_name = f'{sess_name}_segmentations.pt'
    torch.save(segmentations, os.path.join(cached_dir, segmentations_name))
    ...
    embeddings_name = f'{sess_name}_embeddings.pt'
    torch.save(embeddings, os.path.join(cached_dir, embeddings_name))
    ...
    # rttm_gt = load_rttm(rttm_file)
    # ann = rttm_gt[sess_name]
    # hard_clusters = oracle_HA_per_window(ann, segmentations.data)
...
if __name__ == "__main__":
    ...
    diarization_pipeline = SpeakerDiarizationPipeline(
        ...
        device=torch.device("cuda:0") if torch.cuda.is_available() else torch.device("cpu")
        # TODO: device anpassen wenn fehler cuda:0 nicht verfügbar ist
    )
