...
recipe_root=/home/ameise/Code/DiariZen/recipes/diar_ssl_mc
export exp_root=/net/vol/ameise/Diarizen/exp
...
dscore_dir=/net/vol/deegen/pythonuserbasepromotion/dscore
...
if [ $stage -le 1 ]; then
    ...
    source  /net/vol/ameise/Diarizen/.diarizen_env/bin/activate && python run_dual_opt_mc_lazy.py -C $train_conf -M train $resume_flag
...
diarizen_hub=/net/vol/deegen/models/hub/models--BUT-FIT--diarizen-wavlm-large-md-s80/snapshots/50167e9a5243663ff51777823ff7d53da7b87166
...
if [ ! -e $embedding_model ]; then
    ...
    embedding_model=/net/vol/deegen/models/pyannote/wespeaker-voxceleb-resnet34-LM/pytorch_model.bin
fi
...
#    cat $train_log | grep 'Loss/DER' | awk -F ']:' '{print $NF}' > $diarization_dir/val_metric_summary.lst
    ...
    # #source  /scratch/hpc-prf-nt2/deegen/deploy/forschung/DiariZen/.diarizen/bin/activate 
    source /net/vol/ameise/Diarizen/.diarizen_env/bin/activate && python infer_avg_oracle.py -C $config_dir \
    ...
    for collar in 0 0.25; do
        #source  /scratch/hpc-prf-nt2/deegen/deploy/forschung/DiariZen/.diarizen/bin/activate && 
        source /net/vol/ameise/Diarizen/.diarizen_env/bin/activate && python ${dscore_dir}/score.py \
    ...
    for collar in 0 0.25; do
    #source  /scratch/hpc-prf-nt2/deegen/deploy/forschung/DiariZen/.diarizen/bin/activate 
    source /net/vol/ameise/Diarizen/.diarizen_env/bin/activate && python -m diarizen.scoring.der_ov \
                    --storage_dir $OUT_DIR \
