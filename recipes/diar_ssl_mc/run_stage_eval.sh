...
export train_conf=$conf_dir/spk_count_linear_noisy_to_gcpsd_encoder_ffn_film_all_layers_finetune_test.toml # gcc_encoder_precomputed  baseline_sc_debug.toml   #    mc_wavlm_updated_conformer.toml #MC
...
if [ $stage -le 1 ]; then
    if (! $use_dual_opt); then
        ...
        micromamba activate diarizen && CUDA_VISIBLE_DEVICES="0,1,2,3" accelerate launch \
            --num_processes 4 --main_process_port 1136 \
            run_dual_opt_mc.py -C $train_conf -M train
#########            ### Debugguing: use only one GPU and worker
#        micromamba activate diarizen && CUDA_VISIBLE_DEVICES="4" accelerate launch \
...
#        micromamba activate diarizen && python count_overlap.py \
#            -i ${data_dir}/${dtype}/${dset}/wav.scp \
#            -o ${data_dir}/${dtype}/${dset} \
#            -dset ${dset} \
#            --rttm /mnt/matylda5/qdeegen/deploy/forschung/DiariZen/recipes/diar_ssl_mc/data_no_chime/train/rttm  # ${data_dir}/${dtype}/${dset}/rttm
