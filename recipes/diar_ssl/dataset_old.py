...
def _collate_fn(batch, max_speakers_per_chunk=4) -> torch.Tensor:
    ...
    for x, y, name in batch:
        ...
        # collated_x.append(x[0][None,:]) # when MC datasets have different number of mics => np.stack shape mismatch
        collated_x.append(x)
        # But long-term anyway new way => maybe no np.stack but list of examples? ecah list MC recording can have differnt shape then
        collated_y.append(y)
    ...
    try:
        ...
        # Eigene Fehlermeldung zusammenbauen
        msg = f"Fehler beim Stapeln der Arrays mit np.stack: {e}\n"
        msg += f"Anzahl der Arrays: {len(collated_x)}\n"

        # Beispiel: Shapes der ersten paar Arrays ausgeben
        shapes = [arr.shape for arr in collated_x]
        msg += f"Shapes der ersten 5 Arrays: {shapes}\n Jetzt kommt y \n \n"

        # Optional: Infos über dtype, falls relevant
        dtypes = [arr.shape for arr in collated_y]
        msg += f"Dtype der ersten 5 Arrays: {dtypes}\n"
