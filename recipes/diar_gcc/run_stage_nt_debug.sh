...
recipe_root=/net/vol/deegen/deploy/forschung/DiariZen/recipes/diar_gcc
...
##############            ### Debugguing: use only one GPU and worker
         micromamba activate diarizen && CUDA_VISIBLE_DEVICES="0,1" accelerate launch \
            --num_processes 2 --main_process_port 1137 \
            run_dual_opt_mc_lazy.py -C $train_conf -M train
#          micromamba activate diarizen
#          CUDA_VISIBLE_DEVICES="0"
#          echo "Running dual-opt training with 1 GPU for debugging..."
#          echo $CUDA_VISIBLE_DEVICES
#          python \
#            run_dual_opt_mc_lazy.py -C $train_conf -M train  2>&1 | tee training.log
    fi
fi
