...
def _collate_fn(batch, max_speakers_per_chunk=4, th=0.15, gcpsd=False) -> torch.Tensor:
    ...
    for x, y, name, gcc in batch:
        ...
        num_spk = np.sum(y, axis=-1, keepdims=True) # sum over spk axis, get number of active spk per frame

        # ov = ov # np.clip(num_spk, 1, 2)  # limit to 0, 1, or 2; values above 2 are mapped to 2
        # # ov_ratio = np.sum(ov)/len(ov)
        # # if ov_ratio >= th:
        # #     ov = 1
        # # else:
        # #     ov = 0
    ...
    if gcpsd:
        return {
            ...
            # "mag": torch.from_numpy(np.stack(magnitudes)).float(),
        }
...
class DiarizationDataset(Dataset):
    def __init__(
        ...
        load_gcc_dir = None,
        subset = "train",
        num_channels = 4,  # number of channels for multichannel moderesample
        resample = False,
        modelbased = False,
        gcpsd = False,
    ):
    ...
    def load_gccs(self, path, start, end, load_gcc_dir, kmin=None, kmax=None):
        ...
        with h5py.File(h5_filename, 'r') as f:
            ...
            if kmax:
                gcc_features = grp[segment_id][:, :, kmin:kmax]
            else:
    ...
    def __getitem__(self, idx):
        ...
        while True:
            ...
            # if re.match(r"^S\d{2}$", session):
            #     path = self.chime_array_selection(session, path)
        ...
        # print(chunked_annotations['label_idx'], "labels:", labels, flush=True)
        ...
        if self.load_gcc_dir == "base":
            ...
            if self.gcpsd:
                ...
                data_padded = self.pad_data(data, size=fft_size)
                sigs_stft = pb.transform.stft(data_padded[0], size=fft_size, shift=320,
                                              pad=False, fading=False)
            ...
            gcc_features = self.compute_gcc(data_padded, frame_size_gcc=fft_size, frame_shift_gcc=320, f_max_gcc=fmax, f_min=fmin,
        ...
        if self.resample:
            th = 1.70
            # n_spk = self.count_active_speakers_intervals(chunk_start, chunk_end, chunked_annotations)
            num_spk = np.sum(mask_label, axis=-1, keepdims=True)
            ov = np.clip(num_spk, 1, 2)  # limit to 0, 1, or 2; values above 2 are mapped to 2
            ov_ratio = np.sum(ov)/len(ov)
            if ov_ratio >= th:
                overlap_flag = 1
            else:
                overlap_flag = 0
            return data, mask_label, session, gcc_features, overlap_flag
        else:
