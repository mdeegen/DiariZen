import glob
import numpy as np
from trainer_dual_opt import Trainer
...
def run(config, resume):
    ...
    # print("Trainer initialized.",trainer.state.epochs_trained, flush=True)
    checkpoint_dir = trainer.exp_dir / "checkpoints"

    # ckps = [p for p in checkpoint_dir.glob("epoch*")]
    # print("Found checkpoints: ", ckps, flush=True)
    ckps = sorted([p for p in checkpoint_dir.glob("epoch*")])
    # print("Found checkpoints: ", ckps, flush=True)

    with torch.no_grad():
        for ckp in ckps:
            if str(ckp).endswith("0000"):
                print("Skipping checkpoint ", ckp, flush=True)
                continue
            print("Loading checkpoint from ", ckp, flush=True)
            trainer._load_checkpoint(ckpt_path=str(ckp))
            # print("Trainer initialized.",trainer.state.epochs_trained, flush=True)
            #
            # score = trainer.validate(validate_dataloader)
            trainer.num_correct = 0
            trainer.num_total = 0
            trainer.num_correct_ov = 0
            trainer.num_total_ov = 0

            score = trainer.validate(validate_dataloader)
            if trainer.accelerator.is_local_main_process:
                should_stop = trainer._run_early_stop_check(score)
            logger.info("Validation finished.")

# bash run_stage_noctua2_lazy_ckpts.sh wavlm_counting_first
